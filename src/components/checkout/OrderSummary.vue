<template>
    <div class="bg-card rounded-lg border border-border p-3 sm:p-4 sticky top-4">
        <h2 class="text-base sm:text-lg font-medium text-foreground mb-3 sm:mb-4">
            {{ t('cart.orderSummary') }}
        </h2>
        ...
        <!-- Coupon Section -->
        <div class="mb-3 sm:mb-4 p-2 sm:p-3 bg-accent/30 rounded-lg">
            <div v-if="!appliedCoupon">
                ...
                <div class="flex flex-col sm:flex-row gap-2">
                    <input
                        ...
                        class="flex-1 px-2 sm:px-3 py-2 text-sm border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary min-w-0"
                    />
                    <button
                        @click="applyCoupon"
                        :disabled="!couponCode || isApplyingCoupon"
                        class="px-2 sm:px-3 py-2 text-sm bg-primary text-primary-foreground rounded-lg hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed transition-colors whitespace-nowrap w-full sm:w-auto"
                    >
                        {{
                            isApplyingCoupon
            ...
            <div v-else class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                ...
                <button
                    @click="removeCoupon"
                    class="text-xs sm:text-sm text-destructive hover:text-destructive/80 whitespace-nowrap self-start sm:self-auto"
                >
